...
let allCaps = runners.map((runner) => `${ runner.first_name.toUpperCase() } ${runner.last_name.toUpperCase()}`);
console.log(allCaps);
...
// Problem 1
// The event director recently watched a video by Gary Vaynerchuk on his social media newsfeed. He decided that he wants to become an entrepreneur and sell his new protein powder supplement to all of the runners. Return a new array containing each runner's first name and email.
const emailList = runners.map((runner) => `${ runner.first_name } ${runner.email}`);
console.log(emailList);

// Problem 2
// The event director was fired for abusing the email list to health supplement after Problem 1. The former event director needs to network with university graduates in order to find a new job. Return a new array that only contains emails that include.edu.

const eduEmailList = runners.filter((runner) => {
    return runner.email.includes('.edu');
});
console.log(eduEmailList);

// Problem 3
// The event director got his job back to assign Problem 3. The director needs a list of company names that are sorted alphabetically.

const companyList = runners.map(runner => runner.company_name).sort();
